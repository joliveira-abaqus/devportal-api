...
describe('Health Check', () => {
  ...
  it('deve retornar 200 com todos os checks "ok" quando tudo está funcionando', async () => {
  ...
  it('deve retornar 503 com database "error" quando o banco falha', async () => {
  ...
  it('deve retornar 503 com redis "error" quando o Redis falha', async () => {
    (mockPrisma.$queryRaw as jest.Mock).mockResolvedValue([{ '?column?': 1 }]);
  ...
  it('deve retornar 503 quando ambos banco e Redis falham', async () => {
    (mockPrisma.$queryRaw as jest.Mock).mockRejectedValue(new Error('DB down'));
    (mockRedis.ping as jest.Mock).mockRejectedValue(new Error('Redis down'));
